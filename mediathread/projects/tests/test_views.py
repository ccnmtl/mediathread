...
class ProjectViewTest(MediathreadTestMixin, TestCase):
    ...
    def test_project_save_valid(self):
        ...
        self.assertEqual(sum(1 for v in versions), 1)
        ...
        self.assertEqual(the_json["status"], "success")
        self.assertFalse(the_json["is_essay_assignment"])
        self.assertEqual(the_json["title"], "Private Student Essay")
        self.assertEqual(the_json["revision"]["visibility"], "Draft")
        self.assertIsNone(the_json["revision"]["public_url"])
        self.assertEqual(the_json["revision"]["due_date"], "")

        project = Project.objects.get(id=self.project_private.id)
        self.assertEqual(project.author, self.student_one)
        self.assertIn(self.student_one, project.participants.all())
        self.assertIn(self.student_two, project.participants.all())

        versions = Version.objects.get_for_object(
            self.project_private).get_unique()
        self.assertEqual(sum(1 for v in versions), 2)
    ...
    def test_project_save_swap_authors(self):
        ...
        project = Project.objects.get(id=self.project_private.id)
        self.assertEqual(project.author, self.student_two)
    ...
    def test_project_save_invalid_title(self):
        ...
        the_json = json.loads(response.content)
        self.assertEqual(the_json["status"], "error")
    ...
    def test_sequence_assignment_response_delete(self):
        ...
        self.assertEqual(response1.assignment(), sequence_assignment)
    ...
    def test_project_workspace(self):
        ...
        response = self.client.get(url, {})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['project'], self.project_private)
        self.assertEqual(response.context['space_owner'], 'student_one')
    ...
    def test_project_workspace_ajax(self):
        ...
        self.assertEqual(the_json['space_owner'], 'student_one')

        self.assertEqual(len(the_json['panels']), 2)

        panel = the_json['panels'][0]
        self.assertFalse(panel['is_faculty'])
        self.assertEqual(len(panel['owners']), 6)
        self.assertEqual(panel['vocabulary'], [])
        self.assertEqual(panel['template'], 'project')
        self.assertEqual(len(panel['context']['annotations']), 2)
        self.assertEqual(len(panel['context']['assets']), 1)
        ...
        self.assertEqual(panel['context']['response_count'], 0)
        self.assertEqual(panel['context']['responses'], [])
        self.assertEqual(panel['context']['type'], 'project')
    ...
    def test_project_workspace_collaborator(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['project'], self.project_private)
        self.assertEqual(response.context['space_owner'], 'student_two')
...
class TestProjectSortView(MediathreadTestMixin, TestCase):
    ...
    def test_sort(self):
        ...
        self.assertEqual(project.ordinality, 1)

        project = Project.objects.get(id=project2.id)
        self.assertEqual(project.ordinality, 0)
...
class SelectionAssignmentViewTest(MediathreadTestMixin, TestCase):
    ...
    def test_get_my_response(self):
        ...
        responses = self.assignment.responses(self.sample_course,
                                              self.student_one)
        self.assertEqual(response, view.get_my_response(responses))
    ...
    def test_get_peer_response(self):
        ...
        self.assertEqual(response, view.get_peer_response(response))
    ...
    def test_get_context_data(self):
        ...
        self.assertEqual(ctx['my_response'], response)
        self.assertEqual(ctx['the_response'], response)
        self.assertEqual(ctx['item'], self.asset)
        self.assertEqual(ctx['response_view_policies'], RESPONSE_VIEW_POLICY)
        self.assertEqual(ctx['submit_policy'], 'CourseProtected')
...
class SelectionAssignmentEditViewTest(MediathreadTestMixin, TestCase):
    ...
    def test_save(self):
        ...
        self.assertEqual(project.body, 'Body Text')
        self.assertEqual(project.assignmentitem_set.count(), 1)
        self.assertEqual(project.assignmentitem_set.first().asset, asset1)
...
class SequenceAssignmentViewTest(MediathreadTestMixin, TestCase):
    ...
    def test_get_context_data_for_assignment(self):
        ...
        self.assertEqual(ctx['my_response'], response)
        self.assertEqual(ctx['the_response'], response)
        self.assertEqual(ctx['responses'], [response])
    ...
    def test_get_context_data_a_response(self):
        ...
        self.assertFalse(ctx['assignment_can_edit'])
        self.assertFalse(ctx['response_can_edit'])
        self.assertEqual(ctx['my_response'], my_response)
        self.assertEqual(ctx['the_response'], response)
        self.assertEqual(ctx['responses'], [my_response])
        self.assertTrue(ctx['show_instructions'])
    ...
    def test_public_view(self):
        ...
        self.assertEqual(response.context['the_response'],
                         assignment_response)
        ...
        self.assertEqual(response.context['responses'], [])
...
class ProjectItemViewTest(MediathreadTestMixin, TestCase):
    ...
    def assert_visible_notes(self, viewer, visible,
        ...
        for idx, note in enumerate(notes):
            self.assertEqual(note['id'], visible[idx][0])
            self.assertEqual(note['editable'], visible[idx][1])
...
class ProjectListViewTest(MediathreadTestMixin, TestCase):
    ...
    def test_get_project_owner_self(self):
        ...
        self.assertEqual(view.get_project_owner(), self.student_one)
    ...
    def test_get_project_owner_alt_course(self):
        ...
        with self.assertRaisesRegex(Http404, 'not enrolled'):
            view.get_project_owner()
    ...
    def test_get(self):
        ...
        self.assertEqual(ctx['object_list'].count(), 0)
        self.assertEqual(ctx['owner'].username, 'student_one')
        self.assertEqual(ctx['sortby'], 'title')
        self.assertEqual(ctx['direction'], 'asc')
        self.assertEqual(ctx['user_has_projects'], False)
    ...
    def test_get_sorted(self):
        ...
        self.assertEqual(response.status_code, 200)
        ctx = response.context_data
        self.assertEqual(ctx['object_list'].count(), 2)
        self.assertEqual(ctx['object_list'][0], p1)
        self.assertEqual(ctx['object_list'][1], p2)
        self.assertEqual(ctx['sortby'], 'title')
        self.assertEqual(ctx['direction'], 'asc')
        self.assertEqual(ctx['user_has_projects'], True)
        ...
        ctx = response.context_data
        self.assertEqual(ctx['object_list'].count(), 2)
        self.assertEqual(ctx['object_list'][0], p2)
        self.assertEqual(ctx['object_list'][1], p1)
        self.assertEqual(ctx['direction'], 'desc')
...
class AssignmentListViewTest(MediathreadTestMixin, TestCase):
    ...
    def test_get(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context_data['status'], 'all')
    ...
    def test_sort_by_full_name(self):
        ...
        ctx = response.context_data
        self.assertEqual(ctx['object_list'].count(), 2)
        self.assertEqual(ctx['object_list'][0], a1)
        self.assertEqual(ctx['object_list'][1], a2)
        self.assertEqual(ctx['sortby'], 'full_name')
        self.assertEqual(ctx['direction'], 'asc')
        ...
        ctx = response.context_data
        self.assertEqual(ctx['object_list'].count(), 2)
        self.assertEqual(ctx['object_list'][0], a2)
        self.assertEqual(ctx['object_list'][1], a1)
        self.assertEqual(ctx['sortby'], 'full_name')
        self.assertEqual(ctx['direction'], 'desc')
