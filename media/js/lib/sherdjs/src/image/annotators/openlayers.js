    ...
    Sherd.Image.Annotators.OpenLayers = function() {
        ...
        this.setState = function(obj, options) {
            if (typeof obj === 'object') {
                //because only one annotation is allowed at once.  At
                //the moment, we could do a better job of saving 'all'
                //features / in an annotation rather than overwriting
                //with the last one / but then we run into confusion
                //where people think they're making / a lot of
                //annotations, but really made one.
                ...
                if (options && options.mode && options.mode === 'reset') {
                    ...
                    if (self.openlayers.editingtoolbar) {
                        if (
                            ...
                            // whole asset view. no annotations. or,
                            // just browsing
        ...
        this.initialize = function(create_obj) {
            if (!self.openlayers.editingtoolbar) {
                ...
                self.targetview.openlayers.map.addControl(
                    self.openlayers.editingtoolbar);
                self.openlayers.editingtoolbar.deactivate();

                //Q: this doubles mousewheel listening, e.g. why did
                //we need it?
                //A: needed for not showing toolbar until clicking on
                //an annotation
            ...
            self.events.connect(self.components.center, 'click', function(evt) {
                self.targetview.setState({
                    feature: self.targetview.currentfeature});
            });
        };
        this.openlayers = {
            CustomEditingToolbar:
            OpenLayers.Class(OpenLayers.Control.EditingToolbar, {
                    ...
                    this.sherd.squareHandler =
                        new OpenLayers.Control.DrawFeature(
                            ...
                            OpenLayers.Handler.RegularPolygon, {
                                ...
                                //note, this is not supported by
                                //openlayers--we need to make our own
                                //icons
            ...
            'create': function() {
                ...
                return {
                    ...
                    '<div id="' + id + '">' +
                        ...
                        'To create a selection of an image, choose a ' +
                        'drawing tool, located on the upper, ' +
                        'right-hand side of the image. The polygon tool ' +
                        'works by clicking on the points of ' +
                        'the polygon and then double-clicking the last ' +
                        'point.<br /><br />' +
                        'Add title, tags and notes. If a Course Vocabulary ' +
                        'has been enabled by ' +
                        'the instructor, apply vocabulary terms. Click ' +
