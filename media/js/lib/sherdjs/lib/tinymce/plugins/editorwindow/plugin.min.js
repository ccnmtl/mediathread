 ...
 * $Id
 *
 * @author Schuyler Duveen at Columbia Center for New Media Teaching
 * and Learning
...
(function() {
    ...
    tinymce.create('tinymce.plugins.EditorWindow', {
        ...
        _nodeChanged: function(ed, cm, currentElt, collapsed, opt) {
            ...
            if (!currentElt.classList.contains('materialCitation')) {
                // Only show the editor window for material citations
                return false;
            }
                ...
                if (parent) {
                    if (this.opened) {
                        if (parent === this.tag_for_window) {
                            ...
                            ///- also impedes real supercession
                            ///- (e.g. A.cls beats link)
                            this._closeWindow();
                        }
                    }
        ...
        _positionWindow: function(ed, currentElt, parent) {
            const pos = this.getAbsoluteCursorPos(ed, currentElt, parent);
            const rect = DOM.getRect(this.win);
            const viewport = DOM.getViewPort(window);

            let top = 0;
            if (jQuery) {
                top = jQuery(currentElt).offset()?.top;
            }
        ...
        _openWindow: function(ed, currentElt, parent) {
            ...
            if (jQuery && jQuery('.essay-space-container').length > 0) {
                jQuery('.essay-space-container').first().append(this.win);
            } else {
