...
function showMessage(msg,onclose){jQuery("#dialog-confirm").html(msg);jQuery("#dialog-confirm").dialog({resizable:false,modal:true,title:"Success",close:function(){if(onclose){onclose();jQuery("#dialog-confirm").html("");}},buttons:{"OK":function(){jQuery(this).dialog("close");}}});}
...
var tag_field=frm.elements['annotation-tags'];if(tag_field){var tags=tag_field.value.split(',');for(var i=0;i<tags.length;i++){if(tags[i].trim().length>25){var msg='The '+tags[i]+' is too long. Tags should be less than 25 characters. '+'And, be sure to separate your tags with commas.';showMessage(msg,function(){jQuery(saveButton).removeAttr("disabled");jQuery(saveButton).removeClass("saving");jQuery(saveButton).attr("value","Save");});return;}}}
if(frm.elements['annotation-title'].value===''){showMessage('Please specify a selection title',function(){jQuery(saveButton).removeAttr("disabled");jQuery(saveButton).removeClass("saving");jQuery(saveButton).attr("value","Save");});return;}
...
jQuery("a.linkRespond").unbind("click");jQuery("a.btnRespond").unbind("click");jQuery("a.btnDeleteResponse").unbind("click");jQuery.ajax({url:url,dataType:'json',cache:false,success:function(the_records){self.update(the_records);jQuery("a.btnRespond").bind("click",function(evt){self.createAssignmentResponse(evt);});jQuery("a.linkRespond").bind("click",function(evt){self.createAssignmentResponse(evt);});jQuery("a.btnDeleteResponse").bind("click",function(evt){self.deleteAssignmentResponse(evt);});}});};ProjectList.prototype.selectOwner=function(username){var self=this;var url=username?MediaThread.urls['your-projects'](username):MediaThread.urls['all-projects']();jQuery.ajax({type:'GET',url:url,dataType:'json',error:function(){showMessage('There was an error retrieving the project list.');},success:function(the_records){self.update(the_records);}});return false;};ProjectList.prototype.getShowingAllItems=function(json){return!json.hasOwnProperty('space_owner');};ProjectList.prototype.getSpaceUrl=function(active_tag,active_modified){var self=this;if(self.getShowingAllItems(self.current_records)){return MediaThread.urls['all-projects']();}else{return MediaThread.urls['your-projects'](self.current_records.space_owner.username);}};ProjectList.prototype.updateSwitcher=function(){var self=this;self.switcher_context.display_switcher_extras=!self.switcher_context.showing_my_items;Mustache.update("switcher_collection_chooser",self.switcher_context,{parent:self.parent});jQuery(self.parent).find("a.switcher-choice.owner").unbind('click').click(function(evt){var srcElement=evt.srcElement||evt.target||evt.originalTarget;var bits=srcElement.href.split("/");var username=bits[bits.length-1];if(username==="all-class-members"){username=null;}
...
self.current_records=the_records;var n=_propertyCount(the_records.active_filters);if(n>0){the_records.active_filter_count=n;}
Mustache.update(self.template_label,the_records,{parent:self.parent,pre:function(elt){jQuery(elt).hide();},post:function(elt){self.updateSwitcher();jQuery(elt).fadeIn("slow");}});};var ProjectPanelHandler=function(el,parent,panel,space_owner){var self=this;self.el=el;self.panel=panel;self.projectModified=false;self.parentContainer=parent;self.space_owner=space_owner;self.tiny_mce_settings=tiny_mce_settings;djangosherd.storage.json_update(panel.context);if(panel.context.can_edit){var select=jQuery(self.el).find("select[name='participants']")[0];jQuery(select).addClass("selectfilter");SelectFilter.init("id_participants_"+panel.context.project.id,"participants",0,"/media/admin/");var assignment_elt=jQuery(self.el).find("label[for='id_publish_2']").parent();if(assignment_elt.length>0){jQuery(self.el).find('div.due-date').appendTo(assignment_elt);}}
...
var element=jQuery(self.el).find(".response-list")[0];jQuery(element).dialog({buttons:[{text:"Cancel",click:function(){jQuery(this).dialog("close");}},{text:"View Response",click:function(){self._save=true;jQuery(this).dialog("close");}}],beforeClose:function(event,ui){if(self._save){self._save=false;var opts=jQuery(self.el).find("select[name='responses'] option:selected");if(opts.length<1){showMessage("Please select a response");return false;}else{var val=jQuery(opts[0]).val();window.location=val;}}
...
jQuery(self.el).find("select[name='participants'] option").attr("selected","selected");var data=jQuery(frm).serializeArray();data=data.concat(jQuery(document.forms.editparticipants).serializeArray());var saveButton=jQuery(self.el).find(".project-savebutton").get(0);jQuery(saveButton).attr("disabled","disabled").attr("value","Saving...").addClass("saving");jQuery.ajax({type:'POST',url:frm.action,data:data,dataType:'json',error:function(){jQuery(saveButton).removeAttr("disabled").attr("value","Save").removeClass("saving");showMessage('There was an error saving your project.');},success:function(json,textStatus,xhr){if(json.status==='error'){showMessage(json.msg);}else{var lastVersionPublic=jQuery(self.el).find(".last-version-public").get(0);if(json.revision.public_url){jQuery(lastVersionPublic).attr("href",json.revision.public_url);jQuery(lastVersionPublic).show();}else{jQuery(lastVersionPublic).attr("href","");jQuery(lastVersionPublic).hide();}
...
jQuery(self.el).find("td.panel-container").toggleClass("media collection");jQuery(self.el).find("td.panhandle-stripe").toggleClass("media collection");jQuery(self.el).find("div.pantab").toggleClass("media collection");jQuery(window).trigger("resize");return false;};function ajaxDelete(link,container,opts){var postUrl=null;if(link&&link.href){postUrl=link.href;}else if(opts&&opts.href){postUrl=opts.href;}else{showMessage("An error occurred. Unable to delete this item.");}
...
jQuery("#dialog-confirm").html(msg);jQuery("#dialog-confirm").dialog({resizable:false,height:140,modal:true,title:"Confirm action",close:function(event,ui){jQuery(dom).removeClass('about-to-delete');},buttons:{"Cancel":function(){jQuery(this).dialog("close");},"OK":function(){jQuery(this).dialog("close");jQuery.ajax({type:'POST',url:postUrl,success:function(responseText,textStatus,xhr){if(xhr.status===200){jQuery(dom).fadeOut(function(){jQuery(dom).remove();});}else{showMessage("Error: "+textStatus);}
if(opts&&opts.success){opts.success();}},error:function(xhr){window.sky=xhr;showMessage("Error!");}});}}});return false;}