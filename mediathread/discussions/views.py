...
from mediathread.mixins import (
    LoggedInCourseMixin, LoggedInFacultyMixin, AjaxRequiredMixin,
    JSONResponseMixin
)
...
class DiscussionSaveCommentView(LoggedInCourseMixin, AjaxRequiredMixin,
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not comment.content_object.permission_to(
                'manage', request.course, request.user):
            return HttpResponseForbidden(
                'You do not have permission to edit this discussion.')

        # update the body of the comment. (confusingly called "comment")
        comment.comment = request.POST['comment']
