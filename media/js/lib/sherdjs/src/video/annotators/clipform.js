...
if (!Sherd.Video.Annotators.ClipForm) {
    Sherd.Video.Annotators.ClipForm = function() {
        // @todo -- consider moving these functions out of Video
        // and into a separate Utilities or Helpers file?
        ...
        // it were a compilation of information from both clipform & player
        // itself
        ...
        this.setState = function(obj, options) {
            if (typeof obj === 'object') {
                ...
                if (options && options.tool_play) {
                    self.unbind.tool_play = self.events.connect(
                        options.tool_play, 'click', function(evt) {
                            var obj = self.getState();
                            self.events.signal(self.targetview, 'playclip', {
                                start: obj.start, end: obj.end, autoplay: true
                            });
                        });
                }
                ...
                if (typeof start !== 'undefined') {
                    ...
                    self.events.signal(self.targetview, 'clipstart', {
                        start: codeToSeconds(start)
                    });
                }
                if (typeof end !== 'undefined') {
                    ...
                    self.events.signal(self.targetview, 'clipend', {
                        end: codeToSeconds(end)
                    });
                }
        ...
        this.initialize = function(create_obj) {
            var postStartButton = function(movieTime) {
                ...
                if (
                    self.components.endField &&
                        (movieTime > codeToSeconds(
                            self.components.endField.value))
                ) {
            ...
            self.events.connect(
                self.components.startButton, 'click', function(evt) {
                    if (
                        typeof self.targetview.media.getAsyncTime ===
                            'function'
                    ) {
                        self.targetview.media.getAsyncTime().then(
                            function(time) {
            ...
            var postEndButton = function(movieTime) {
                ...
                if (self.targetview.media.pause) {
                    ///due to overwhelming user feedback for it to
                    ///pause on clicking end-button
                ...
                // if the start time is greater then the endtime, make
                // start time match end time
                if (
                    self.components.startField &&
                        (movieTime < codeToSeconds(
            ...
            self.events.connect(
                self.components.endButton, 'click', function(evt) {
                    if (
                        typeof self.targetview.media.getAsyncTime ===
                            'function'
                    ) {
            ...
            self.events.connect(
                self.components.startField, 'change', function(evt) {
                    var obj = self.getState();

                    // if the start time is greater then the endtime, make
                    // end time match start time
            ...
            self.events.connect(
                self.components.endField, 'change', function(evt) {
                    var obj = self.getState();
            ...
            self.events.connect(
                self.components.playClip, 'click', function(evt) {
                    var obj = self.getState();
                    self.events.signal(self.targetview, 'playclip', {
        ...
        this.microformat.create = function(obj) {
            ...
            return {
                ...
                'Create a selection by clicking Start Time and End Time ' +
                    'buttons as the video plays, ' +
                    'or by manually typing in times in the associated ' +
                    'edit boxes.<br /><br />' +
                    'Add title, tags and notes. If a Course Vocabulary has ' +
                    'been enabled by the instructor, ' +
                    'apply vocabulary terms. Click Save when ' +
                    'you are finished.' +
                    '</p>' +
                    '<table>' +
                    '<tr><td span="0"><div><label for="annotation-title">' +
                    'Selection Times' +
                    '</label></div></td></tr>' +
                    '<tr class="sherd-clipform-editing">' +
                    '<td>' +
                    '<input type="button" class="btn btn-primary btn-sm" ' +
                    'value="Start Time" id="btnClipStart"/> ' +
                    '</td>' +
                    '<td width="10px">&nbsp;</td>' +
                    '<td>' +
                    '<input type="button" class="btn btn-primary btn-sm" ' +
                    'value="End Time" id="btnClipEnd"/> ' +
                    '</td>' +
                    '<td>&nbsp;</td>' +
                    '</tr>' +
                    '<tr class="sherd-clipform-editing">' +
                    '<td>' +
                    '<input type="text" class="timecode" id="clipStart" ' +
                    'aria-label="clip start" value="' +
                    ...
                    '<td>' +
                    '<input type="text" class="timecode" id="clipEnd" ' +
                    'aria-label="clip end" value="' +
                    self.components.end +
                    '" />' +
                    ...
                    'class="regButton videoplay" id="btnPlayClip" src="' +
                    STATIC_URL +
                    'img/icons/meth_video_play.png"/>' +
        ...
        this.microformat.components = function(html_dom, create_obj) {
            return {
                ...
                'startFieldDisplay': document.getElementById(
                    'clipStartDisplay'),
