...
def there_is_not_a_state_name_panel(step, state, name):
    ...
    """
    try:
        panel = world.browser.find_element_by_css_selector("td.panel-container.%s.%s" % (state.lower(), name.lower()))
...
def the_name_panel_has_a_state_subpanel(step, name, state):
    panel = world.browser.find_element_by_css_selector("td.panel-container.open.%s" % name.lower())
...
def the_panel_has_a_name_button(step, panel, name):
    panel = world.browser.find_element_by_css_selector("td.panel-container.open.%s" % panel.lower())
    assert panel != None, "Can't find panel named %s" % panel
...
def the_project_visibility_is_level(step, level):
    elt = world.browser.find_element_by_css_selector("input[name=publish]:checked")
    assert elt != None
...
def there_is_a_comment_that_begins_text(step, text):
    elts = world.browser.find_elements_by_css_selector("div.threaded_comment_text")
...
def i_click_the_link_citation_in_the_panelname_panel(step, link, panelname):
    panel = world.browser.find_element_by_css_selector("td.panel-container.open.%s" % panelname.lower())
    assert panel != None, "Can't find panel named %s" % panel
...
def i_toggle_the_panelname_panel(step, panelname):
    pantab = world.browser.find_element_by_css_selector("div.pantab.%s" % panelname.lower())
