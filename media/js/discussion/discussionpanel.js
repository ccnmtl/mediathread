...
DiscussionPanelHandler.prototype.open_edit = function (evt, focus) {
    ...
    if (evt === undefined) {
        ...
        self.set_comment_content(self.read({ html: jQuery('#comment-' + id).get(0)}));
    ...
    if (xhr.status === 200  && !res.error) {
        if (res.comment_id) {
            ...
            case 'post':
                ...
                if (!parent_html) {
                    parent_html = jQuery(self.el).find('div.threadedcomments-container')[0];
                }
                ...
                jQuery('span.respond_prompt', ul).click(function (evt) { self.open_respond(evt); });
                jQuery('span.edit_prompt', ul).click(function (evt) { self.open_edit(evt); });
                break;
            case 'update':
