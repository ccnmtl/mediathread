...
class TaxonomyViewTest(MediathreadTestMixin, TestCase):
    ...
    def test_simple_association(self):
        ...
        self.assertEqual(term.termrelationship_set.count(), 0)
        ...
        self.assertEqual(related.term, term)
        self.assertEqual(related.sherdnote, self.note)

    def test_removal(self):
        term = Term.objects.get(display_name="Square")

        TermRelationship.objects.create(term=term, sherdnote=self.note)
        self.assertEqual(term.termrelationship_set.count(), 1)
    ...
    def test_removal_and_association(self):
        ...
        self.assertEqual(square.termrelationship_set.count(), 1)
        ...
        self.assertEqual(related.term, circle)
        self.assertEqual(related.sherdnote, self.note)
    ...
    def test_multiple_associations_and_removals(self):
        ...
        self.assertEqual(related_terms[0].term, red)
        self.assertEqual(related_terms[0].sherdnote, self.note)
        self.assertEqual(related_terms[1].term, square)
        self.assertEqual(related_terms[1].sherdnote, self.note)
        ...
        related_terms = TermRelationship.objects.filter(sherdnote=self.note)
        self.assertEqual(len(related_terms), 2)
        self.assertEqual(related_terms[0].term, blue)
        self.assertEqual(related_terms[0].sherdnote, self.note)
        self.assertEqual(related_terms[1].term, circle)
        self.assertEqual(related_terms[1].sherdnote, self.note)
