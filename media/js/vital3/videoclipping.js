...
function startAtStartTime(autoplay) {
	...
	try {
	    ...
	    startTimeoutID=window.setTimeout(function(){tryStart(autoplay);}, 300);
...
function jumpToEndTime() {
    ...
    if (playRate != 0) {stopAtEndTime = false; theMovie.Play(); }
}
...
function jumpToStartTime(autoplay) {
    ...
    stopAtEndTime=true;
    if (!autoplay) { theMovie.SetRate(playRate); }
    if (autoplay || playRate != 0) { theMovie.Play(); }
}
...
function InMovieTime() {
    ...
    if (currtime != '') {
        ...
        if (document.forms['videonoteform'].clipType[1].checked) { //Marker cliptype
            theMovie.Stop();
        }
        initQtDuration();
    }
...
function formToClip() {
    ...
    if (startime.match(/[^0-9:.]/)) throw "Not a valid Start Time";
    if (endtime.match(/[^0-9:.]/)) throw "Not a valid End Time";
...
function testClip() {
    if (clipType == 'Marker') {
	endtime = startime;
	if (typeof(theMovie) != 'undefined' && theMovie.GetPluginStatus() == 'Complete') {
	    startIntTime = codeToInt(startime)-100;
	}
	else {
	    //If marker, then make starttime one second early and then movie forward a second and stop
	    startIntTime = codeToInt(startime)-1000; //1000 milliseconds = 1 second
	}
    }
    else {
	startIntTime = codeToInt(startime);
...
function afterQTreallyLoads() {
    /*
      If we can somehow know when QT has really really
      loaded (not just says it's loaded) and all functions
      return valid values then this is what we would run
      Since we can't do this, most of these are done 'just in time'
...
function initClipStrip() {
    ...
    } catch(err) {logError('clipstrip failed to init',err);}
}
...
function addNoteStrip(noteID, noteclass) {
    ...
    var width = (n.clipType == 'Marker') ? 3 : width=clipStripPos(codeToInt(n.clipEnd))-left;
    var strip=DIV({ 'id':'clipStrip_'+noteID,
		   'class':noteclass,
		   'style':'width:'+width+'px;left:'+left+'px'
		       });
    clipStripTarget.appendChild(strip);
    
    strip.onmouseover=partial(function(t){$('clipStripTitle').innerHTML=t;}, n.title);
    strip.onclick=partial(editNote, noteID);
}

function addNoteStrips() {
    var specialID = (document.forms['videonoteform'].uid) 
	? document.forms['videonoteform'].uid.value 
	: 'noSpecial';
...
function prepareGrabber() {
    ...
    if (!theMovie) theMovie = document.getElementById('movie1');
