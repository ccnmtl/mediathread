...
jQuery(window).trigger("resize");if(self.scrollTop){jQuery(self.parent).find("div.collection-assets").scrollTop(self.scrollTop);self.scrollTop=undefined;}}});};(function(){var global=this;if(window.Mustache){Mustache.set_pragma_default('EMBEDDED-PARTIALS',true);Mustache.set_pragma_default('FILTERS',true);Mustache.set_pragma_default('DOT-SEPARATORS',true);Mustache.set_pragma_default('?-CONDITIONAL',true);MediaThread.urls={'annotation-form':function(asset_id,annotation_id){return'/asset/'+asset_id+'/annotations/'+annotation_id;},'home-space':function(username){return'/?username='+username;},'your-projects':function(username){return'/yourspace/projects/'+username+'/';},'all-projects':function(){return'/yourspace/projects/';},'your-space':function(username,tag,modified,citable){return'/yourspace/'+username+'/asset/?annotations=true'+
(tag?'&tag='+tag:'')+
...
self._update(config,"asset-view-details");self._addHistory(true);var frm=document.forms['annotation-list-filter'];if(frm){frm.elements.showall.checked=hs_DataRetrieve('annotation-list-filter__showall');jQuery(frm.elements.groupby).val(hs_DataRetrieve('annotation-list-filter__group')||'author');jQuery(frm.elements.showall).change(self.showHideAnnotations);jQuery(frm.elements.groupby).change(function(){var val=jQuery(this).val();hs_DataStore('annotation-list-filter__group',val);self.groupBy(val);});self.groupBy(jQuery(frm.elements.groupby).val());}});}};this.showHideAnnotations=function(){var show=document.forms['annotation-list-filter'].elements.showall.checked;hs_DataStore('annotation-list-filter__showall',show||'');if(show){self.layers[self.grouping].show();}else{self.layers[self.grouping].hide();}};this.groupBy=function(grouping){if(this.grouping===grouping||!(self.active_asset&&self.active_asset.id)){return;}
...
jQuery(saveButton).attr("disabled","disabled").attr("value","Saving").addClass("saving");jQuery.ajax({type:'POST',url:frm.action,data:jQuery(frm).serializeArray(),dataType:'json',error:function(){jQuery(saveButton).removeAttr("disabled").attr("value","Saving").removeClass("saving");alert('There was an error saving your changes.');},success:function(json,textStatus,xhr){djangosherd.storage.get({id:json.asset.id,type:'asset',url:MediaThread.urls['asset-json'](json.asset.id,true)},false,function(asset_full){if(json.annotation.creating){jQuery(window).trigger("annotation.on_create",[]);}else{jQuery(window).trigger("annotation.on_save",[]);}
...
jQuery("a.linkRespond").unbind("click");jQuery("a.btnRespond").unbind("click");jQuery("a.btnDeleteResponse").unbind("click");jQuery.ajax({url:url,dataType:'json',cache:false,success:function(the_records){self.update(the_records);jQuery("a.btnRespond").bind("click",function(evt){self.createAssignmentResponse(evt);});jQuery("a.linkRespond").bind("click",function(evt){self.createAssignmentResponse(evt);});jQuery("a.btnDeleteResponse").bind("click",function(evt){self.deleteAssignmentResponse(evt);});}});};ProjectList.prototype.selectOwner=function(username){var self=this;djangosherd.storage.get({type:'asset',url:username?MediaThread.urls['your-projects'](username):MediaThread.urls['all-projects']()},false,function(the_records){self.update(the_records);});return false;};ProjectList.prototype.getShowingAllItems=function(json){return!json.hasOwnProperty('space_owner');};ProjectList.prototype.getSpaceUrl=function(active_tag,active_modified){var self=this;if(self.getShowingAllItems(self.current_records)){return MediaThread.urls['all-projects']();}else{return MediaThread.urls['your-projects'](self.current_records.space_owner.username);}};ProjectList.prototype.updateSwitcher=function(){var self=this;self.switcher_context.display_switcher_extras=!self.switcher_context.showing_my_items;Mustache.update("switcher_collection_chooser",self.switcher_context,{parent:self.parent});jQuery(self.parent).find("a.switcher-choice.owner").unbind('click').click(function(evt){var srcElement=evt.srcElement||evt.target||evt.originalTarget;var bits=srcElement.href.split("/");var username=bits[bits.length-1];if(username==="all-class-members"){username=null;}
return self.selectOwner(username);});};ProjectList.prototype.update=function(the_records){var self=this;self.switcher_context.owners=the_records.owners;self.switcher_context.space_viewer=the_records.space_viewer;self.switcher_context.selected_view=self.selected_view;if(self.getShowingAllItems(the_records)){self.switcher_context.selected_label="All Class Members";self.switcher_context.showing_all_items=true;self.switcher_context.showing_my_items=false;the_records.showing_all_items=true;}else if(the_records.space_owner.username===the_records.space_viewer.username){self.switcher_context.selected_label="Me";self.switcher_context.showing_my_items=true;self.switcher_context.showing_all_items=false;the_records.showing_my_items=true;}else{self.switcher_context.showing_my_items=false;self.switcher_context.showing_all_items=false;self.switcher_context.selected_label=the_records.space_owner.public_name;}
self.current_records=the_records;var n=_propertyCount(the_records.active_filters);if(n>0){the_records.active_filter_count=n;}
Mustache.update(self.template_label,the_records,{parent:self.parent,pre:function(elt){jQuery(elt).hide();},post:function(elt){self.updateSwitcher();jQuery(elt).fadeIn("slow");}});};var ProjectPanelHandler=function(el,parent,panel,space_owner){var self=this;self.el=el;self.panel=panel;self.projectModified=false;self.parentContainer=parent;self.space_owner=space_owner;self.tiny_mce_settings=tiny_mce_settings;djangosherd.storage.json_update(panel.context);if(panel.context.can_edit){var select=jQuery(self.el).find("select[name='participants']")[0];jQuery(select).addClass("selectfilter");SelectFilter.init("id_participants_"+panel.context.project.id,"participants",0,"/media/");}
...
var srcElement=evt.srcElement||evt.target||evt.originalTarget;var frm=srcElement.form;var element=jQuery(frm).find("div.save-publish-status")[0];jQuery(element).dialog({buttons:[{text:"Cancel",click:function(){jQuery(this).dialog("close");}},{text:"Save",click:function(){self._save=true;jQuery(this).dialog("close");}}],beforeClose:function(event,ui){if(self._save){self.saveProject(frm);}
self._save=false;return true;},draggable:true,resizable:false,modal:true,width:430,position:"top",zIndex:10000});jQuery(element).parent().appendTo(frm);return false;};ProjectPanelHandler.prototype.saveProject=function(frm){var self=this;tinyMCE.activeEditor.save();if(!self._validTitle()||!self._validAuthors()){return false;}
jQuery(self.el).find("select[name='participants'] option").attr("selected","selected");var data=jQuery(frm).serializeArray();data=data.concat(jQuery(document.forms.editparticipants).serializeArray());var saveButton=jQuery(self.el).find(".project-savebutton").get(0);jQuery(saveButton).attr("disabled","disabled").attr("value","Saving").addClass("saving");jQuery.ajax({type:'POST',url:frm.action,data:data,dataType:'json',error:function(){jQuery(saveButton).removeAttr("disabled").attr("value","Saving").removeClass("saving");alert('There was an error saving your project.');},success:function(json,textStatus,xhr){var lastVersionPublic=jQuery(self.el).find(".last-version-public").get(0);if(json.revision.public_url){jQuery(lastVersionPublic).attr("href",json.revision.public_url);jQuery(lastVersionPublic).show();}else{jQuery(lastVersionPublic).attr("href","");jQuery(lastVersionPublic).hide();}
...
jQuery(self.el).find('.project-visibility-description').html(json.revision.visibility);self.setDirty(false);self.revision=json.revision;jQuery(saveButton).removeAttr("disabled").removeClass("saving",1200,function(){jQuery(this).attr("value","Save");});}});return true;};ProjectPanelHandler.prototype.setDirty=function(isDirty){var self=this;self.projectModified=isDirty;if(!isDirty&&self.tinyMCE){self.tinyMCE.isNotDirty=1;}};ProjectPanelHandler.prototype.beforeUnload=function(){var self=this;var msg=null;if(self.projectModified||self.tinyMCE.isDirty()||(self.tinyMCE.editorId===tinyMCE.activeEditor.editorId&&tinyMCE.activeEditor.isDirty())){msg="Changes to your project have not been saved.";}else{var title=jQuery(self.el).find("input[name=title]");if(title&&title.length>0){var value=jQuery(title[0]).val();if(!value||value.length<1){msg="Please specify a project title.";}else if(value==="Untitled"){msg='Please update your "Untitled" project title.';}}}
